...
found by machine-checking the lemma in Lean, which also machine-checks the
elementary layer beneath Theorem~\ref{thm:q1}.  All four are recorded in
...
The elementary layer beneath Theorem~\ref{thm:q1} --- Lemma~\ref{lem:amplitude}
and the coefficient bounds --- is machine-checked: $26$ results, no
\texttt{sorry}, standard axioms only.  The formalisation stops below
Theorem~\ref{thm:q1} itself
...
has been formalised and compiles.  The elementary layer beneath
Theorem~\ref{thm:q1} --- Lemma~\ref{lem:amplitude} and the coefficient bounds
it is built on --- has been formalised in Lean~4 against mathlib: $26$ results,
no \texttt{sorry} anywhere in the sources, \texttt{\#print axioms} reporting
\texttt{propext}, \texttt{Classical.choice} and \texttt{Quot.sound} for all of
them and \texttt{sorryAx} for none, with the toolchain and the mathlib revision
pinned in \file{lean/README.md} and the whole development rebuilt and audited
by \file{lean/scripts/check.sh}, which exits non-zero on any of the three
failures.  \emph{The coverage stops below the oscillatory-integral proposition
and below Theorem~\ref{thm:q1} itself} --- Proposition~4.2 and Theorem~5.2 in
the numbering of \file{band-edge-connection.md}, neither attempted: both need a
continuous branch of the Pr\"ufer angle along an ODE-defined curve, which
mathlib does not supply and which is a piece of work in its own right rather
than a translation of the note.  So the machine-checked layer is the bottom of
\S\ref{sec:q1}, not \S\ref{sec:q1}.  It gates nothing here, and it has already
paid for itself once: it is where Lemma~\ref{lem:amplitude}'s hypothesis was
found to be two hypotheses (\S\ref{sec:lemma51}).  \emph{Nothing in it
...
Both hypotheses of Corollary~\ref{cor:upper} are supplied in
\S\ref{sec:unconditional}: the second by Theorem~\ref{thm:q3} and the first by
Theorem~\ref{thm:h0}.  Note what the corollary actually consumes of \eqref{H0},
because it is weaker than the hypothesis and the difference is what made
\eqref{H0} tractable: only $\mu^{-1}\log\|g\|^2\to0$, so \emph{any}
$\|g\|^2\ge e^{-o(\mu)}$ gives the same $-4\pi$.
...
\begin{theorem}[\ours]\label{thm:q2}
...
& \textbf{sign-blind} \\
\S\ref{sec:standards}, \S\ref{sec:lemma51}: the elementary layer is
machine-checked; Lemma~\ref{lem:amplitude}'s hypothesis splits in two
& a compiled proof about an ODE, and a statement about which hypotheses that
proof consumes.  \emph{Formalisation raises confidence in magnitude results and
supplies no direction} --- no sign occurs anywhere in the development
& \textbf{sign-blind} \\
...
\emph{And the same honesty applies to it as to the scripts}: the build was not
re-run on this editing pass, and the counts above are transcribed from the note
that owns it.  What \emph{was} checked here, because it is free, is that no
\texttt{sorry} occurs in any source file and that the axiom audit lists $26$
results; the compilation itself is reproducible by the command above and was
last run by the ticket that produced it.
...
Eight corrections of ours are folded into the text above.  They are listed here
because a reader weighing the tables should know which way the errors ran, and
because the pattern in them is itself a finding.  The last two are new in this
revision, and they are the only two so far that were found by something other
than a person re-reading their own work: one by a machine, one by opening a
source that had been quoted rather than read.
