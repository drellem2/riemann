...
parabolic-cylinder asymptotic, whose standing hypothesis is exactly the regime
we are in.  Fourth, and this is the point, the entire mechanism is
...
& proved & \emph{ours} (\S\ref{sec:lemma51}) \\
...
& proved, on \cite{Dunster2017} & \emph{ours}; the Hermite limit is Dunster's
(\S\ref{sec:h0}) \\
...
\end{quote}
His separation constant is $\lambda_{\mathrm D}=\chi-c^2$, so \emph{his standing
hypothesis $\lambda_{\mathrm D}<0$ is exactly our $\chi_n<c^2$} --- the same
regime hypothesis that Theorem~\ref{thm:q1} and Theorem~\ref{thm:q2} are built
on, and the negation of the one under which the neighbouring literature works
(\S\ref{sec:q1}).  We read the statement and its hypotheses in the source; we
did \emph{not} read the coalescing-turning-point theory his error bounds rest
on, and we have not re-proved any of it.  Everything else in
...
6 & $\|g\|^2\to0.219247199549\ldots$ \eqref{H0}
  & proved, on \cite{Dunster2017} & \S\ref{sec:h0} \\
\hline
\end{tabular}
\end{center}
...
\begin{lemma}[\ours, elementary]\label{lem:amplitude}
Let $\Phi$ satisfy $\big((1-x^2)\Phi'\big)'+(\chi_n-c^2x^2)\Phi=0$ and suppose
$\chi_n<c^2$.  Then $|\Phi(x)|\le|\Phi(1)|$ for all $x\ge1$.
\end{lemma}
...
\[
  V'=\frac{2pp'}{D}-\frac{p^2D'}{D^2}+2\Phi\Phi'
   ...
   =-\,p^2\frac{D'}{D^2}\;\le\;0 ,
\]
the first and third terms cancelling identically because $D=(x^2-1)q$.  $\Phi$
is entire, so $p\to0$ at $x=1$ while $D\to0$ linearly, whence $p^2/D\to0$ and
$V(1^+)=\Phi(1)^2$.  Finally $\Phi^2\le V\le V(1^+)$.
\end{proof}
...
analytic at $x=1$ --- the last only for $V(1^+)=\Phi(1)^2$, and boundedness near
$x=1$ would do, since by Frobenius the exponents there are $0,0$ and the second
solution carries a logarithm.  The hypotheses were tightened after the theorem
...
Hermite limit is imported: \cite{Dunster2017} eq.\ (124) with (107), whose
standing hypothesis $\lambda_{\mathrm D}=\chi-c^2<0$ is exactly $\chi_n<c^2$ ---
the same hypothesis as Theorems \ref{thm:q1} and \ref{thm:q2}, and the negation
of Osipov--Rokhlin's.  Via DLMF 12.7.2,
$U(-n-\tfrac12,z)=e^{-z^2/4}\mathit{He}_n(z)$, his (124) at $x=X/\lambda$ is the
Hermite limit.  His statement and hypotheses were read in the source; the
coalescing-turning-point theory his error bounds rest on was not opened.
\textbf{Gap G13.}  Everything else in the proof --- the window identity, the
...
\emph{Five of the rows above are new since the first draft, and every one of them
is sign-blind.}  That is the strongest form of the thesis available: an entire
...
column.  The sign-bearing rows are exactly the ones they were.
...
\textbf{G13} & \textbf{The one imported input.}  \cite{Dunster2017} eq.\
(124)+(107) is used in Theorem~\ref{thm:h0} and proved by nobody here: its
statement and its hypotheses were read in the source, its error bounds rest on a
coalescing-turning-point theory that was not opened, and no part of it is
re-derived.  It is the natural first target for a second reader, or for
formalisation.  Writing (124) out with the constants tracked, rather than as
$O(\gamma^{-1}\log\gamma)$, is the concrete form of this gap. \\
...
\cite{Dunster2017} (arXiv:1601.00699), where eq.\ (124), eq.\ (107) and the
standing hypothesis $\lambda_{\mathrm D}<0$ were read in the source, and the
coalescing-turning-point theory underlying his error bounds was not
(\textbf{G13}).
...
constant $0.219247199549$ and the window value $0.219246959898$ --- is
transcribed from the note that owns it, with the script named in the table
above.  \emph{The theorems are a different matter and do not have this status}:
...
chain can be read either as it now stands or in the order it was found.  Two earlier
...
other section uses it.  Its guard is that the hypothesis was checked in the
source and converted into our symbols
($\lambda_{\mathrm D}=\chi-c^2<0\Leftrightarrow\chi_n<c^2$), that the author
states on his second page that his earlier paper assumed the opposite sign, and
that the limit it predicts is confirmed numerically to $22$ digits by
\file{verify_h0.py} --- which is corroboration of the \emph{conclusion} and not
of the theorem.  The second-most exposed is the \observed{} column of
...
here; statement and hypotheses read in the source, error-bound theory not
opened --- \textbf{G13}.)}
